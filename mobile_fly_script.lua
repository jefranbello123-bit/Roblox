-- Fly, Speed, ESP, Lock-On y Noclip menú para móviles.
-- Coloca este LocalScript en StarterPlayerScripts o StarterGui.
-- Usa estas funciones sólo en tus propios proyectos; abusar de ellas en juegos públicos puede violar los Términos de Servicio de Roblox.
...
-- Menú principal en dos columnas (más compacto)
...
-- Helper para crear botones con dos columnas
...
-- Botones (Fly, ESP, Speed, Lock, Noclip) en dos columnas y tres filas
...
-- Botones laterales (ascenso/descenso y control de velocidad)
...
-- Variable para decidir si los botones ↑↓ ajustan WalkSpeed o NoclipSpeed
local speedTarget = nil

-- Variables para Lock-On
local lockEnabled = false
local targetCharacter
local lockConnection

-- Función para activar/desactivar colisión de un personaje
...
local function startNoclip()
    ...
    noclipConnection = RunService.RenderStepped:Connect(function()
        ...
        if cam then
            noclipBodyGyro.CFrame = cam.CFrame
        end
    end)
...
-- Lock-On (selecciona el jugador más centrado)
...
speedUpBtn.MouseButton1Click:Connect(function()
    ...
    elseif speedTarget == "noclip" and noclipEnabled then
        local maxNoclipSpeed = 200
        noclipSpeed = math.min(noclipSpeed + speedIncrement, maxNoclipSpeed)
    end
end)
...
speedDownBtn.MouseButton1Click:Connect(function()
    ...
    elseif speedTarget == "noclip" and noclipEnabled then
        local minNoclipSpeed = 10
        noclipSpeed = math.max(noclipSpeed - speedIncrement, minNoclipSpeed)
    end
end)
...
-- También permitimos arrastrar el botón circular directamente.
makeDraggable(openBtn)

print("✅ Fly, ESP, Speed, Lock y Noclip cargados")
