-- Fly, Speed, ESP y Lock-On para móviles con menú compacto en dos columnas.
-- Coloca este LocalScript en StarterPlayerScripts o StarterGui (PlayerGui).
-- Emplea estas funciones solo en tus propios juegos; su abuso en juegos públicos puede violar los Términos de Servicio de Roblox.
...
menuFrame.Position = UDim2.new(0.5, -130, 0.5, -120)
...
-- Helper para crear botones del menú (adaptado a dos columnas)
local function createToggleButton(name, text, pos, color)
    local btn = Instance.new("TextButton", menuFrame)
    btn.Name = name
    btn.Size = UDim2.new(0, 120, 0, 40)  -- ancho reducido para dos columnas
    btn.Position = pos
...
-- Botones de Fly, ESP, Speed y Lock en disposición 2x2
local flyToggleBtn   = createToggleButton("FlyToggle",   "Fly OFF",   UDim2.new(0, 10, 0, 50),  Color3.fromRGB(220, 45, 45))
local espToggleBtn   = createToggleButton("ESPToggle",   "ESP OFF",   UDim2.new(0, 150, 0, 50), Color3.fromRGB(45, 140, 220))
local speedToggleBtn = createToggleButton("SpeedToggle", "Speed OFF", UDim2.new(0, 10, 0, 100), Color3.fromRGB(45, 220, 120))
local lockToggleBtn  = createToggleButton("LockToggle",  "Lock OFF",  UDim2.new(0, 150, 0, 100),Color3.fromRGB(140, 120, 220))

-- Botones laterales para vuelo y velocidad (sin cambios)
...
-- Conexiones para mantener ESP en nuevos jugadores y respawn
...
local function enableESP()
    espEnabled = true
    -- Función local para crear o actualizar el resaltado de un jugador
    ...
    -- Aplicar resaltado a los jugadores actuales y conectar su CharacterAdded
    ...
    -- Conexión global para nuevos jugadores
...
local function disableESP()
    espEnabled = false
    -- Desconectar todas las conexiones de CharacterAdded y la global
...
-- Funciones de Lock-On (ángulo de visión ampliado)
...
-- Limpiar ESP y sus conexiones cuando un jugador abandona
